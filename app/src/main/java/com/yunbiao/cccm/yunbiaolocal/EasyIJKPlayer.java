...
//        }
    ...
    private void loadUri() {
        ...
        if(mPlayMode == MODE_LIST){//判断是否是列表模式
            if(playList == null || playList.size()<=0){
                return;
            }

            if(playIndex >= playList.size()){//判断当前指针
                playIndex = 0 ;
            }

            currPlayUri = playList.get(playIndex);//取出Uri
        }

        if(TextUtils.isEmpty(currPlayUri)){//如果Uri为空则停止
            return;
        }
    ...
    public void enableMouseMode(boolean enable){
        isMouseMode = enable;
    }
    ...
    interface OnFullScreenCallback{
